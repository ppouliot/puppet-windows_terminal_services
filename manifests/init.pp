...
class windows_terminal_services($remote_access){
  ...
  if $::kernel == 'windows' {
    ...
    } notice("Puppet module ${module_name} is ensuring Registry key value HKLM\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server\\fDenyTSConnection is set to ${ts_deny_connections}")
  } else {
