...
class windows_terminal_services($remote_access){
  ...
  if $kernel == 'windows' {
    ...
    } notice("${module_name} Ensuring Registry key HKLM\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server is present")
    registry_value {'HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server\fDenyTSConnection':
      type => string,
      data => $ts_deny_connections,
    } notice("${module_name} Ensuring Registry key value HKLM\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server\\fDenyTSConnection is set to ${ts_deny_connections}")
  } else {
